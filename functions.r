...
jin.MNAR <- function(edata, mv_prop, mnar_ratio, beams = FALSE, batch_factor = NULL, seed=NULL){
  if(!is.null(seed)){
    set.seed(seed)
  }
  Actual_edata = edata
...
DEA_to_plot <- function(input, output){
  output_option = c("TPR", "FPR", "Precision", "Fscore")
  ...
  plot_dat$x <- factor(labels, levels=c("Ground truth","KNN","Mean","MinProb","SVD"))
